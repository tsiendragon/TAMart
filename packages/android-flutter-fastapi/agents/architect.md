...
- 枚举值（用 VARCHAR + CHECK，禁止 MySQL 原生 ENUM）
- 索引：每个 FK 列必须有索引，高频查询列必须有索引
- 外键约束：ON DELETE / ON UPDATE 策略
- 设计决策记录（为什么这样设计）

**步骤 3b：前端 Drift 映射（双侧迁移）**
- 每张可同步/本地缓存的表，在 DATABASE.md「前端 Drift 映射」段给出 Drift table 类与类型映射
- 登记当前 `schemaVersion` 并明确本次需 bump 到 N+1，写出 `MigrationStrategy.onUpgrade` step
- 提醒：schema 变更要 4 件套原子提交（DATABASE.md + Alembic + Drift + 双侧测试），见 AGENTS.md

**步骤 4：等待用户确认**
输出摘要（新增表/修改表/**后端 Alembic + 前端 Drift schemaVersion**/风险），等待用户确认。
