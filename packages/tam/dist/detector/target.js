...
export function detectTargets(cwd, scopeLevel, explicitTargets) {
    ...
    if (scopeLevel === 'user') {
        ...
        if (hasClaude)
            claudeRoot = localClaude;
        if (hasCursor)
            cursorRoot = localCursor;
    }
    const detected = [];
    if (claudeRoot !== null)
    ...
    if (detected.length === 0 && scopeLevel === 'project') {
        throw new TamError(E.NO_TARGET_DETECTED, `No .claude/ or .cursor/ directory found in ${cwd}.\n` +
...
export function getRootForTarget(roots, target) {
    const home = os.homedir();
