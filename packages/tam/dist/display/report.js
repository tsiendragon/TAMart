...
export function printSupportMatrix(ops, skipped) {
    console.log('\n  Support matrix:');
    const rows = [
        ...ops.map(op => ({ assetId: op.assetId, target: op.target, level: op.supportLevel, note: '' })),
        ...skipped.map(s => ({ assetId: s.assetId, target: s.target, level: 'skipped', note: s.reason })),
    ];
...
export function printInstallResult(ops, skipped) {
