...
import type { TamManifest as Manifest } from './schema.js'
...
export function validateManifest(raw: Manifest, pkgDir: string): ValidationIssue[] {
  const issues: ValidationIssue[] = []
  const err = (code: string, msg: string) => issues.push({ code, severity: 'error', message: msg })
  const warn = (code: string, msg: string) => issues.push({ code, severity: 'warning', message: msg })
  ...
  if (!Array.isArray(raw.assets) || raw.assets.length === 0) {
    ...
    for (const asset of raw.assets) {
      ...
      if (asset.type === 'hook') hasExecutable = true
