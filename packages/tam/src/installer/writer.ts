...
import { writeAtomic, ensureDir, readFileSafe } from '../util/fs.js'
import type { FileOp } from '../planner/types.js'

export async function applyOp(op: FileOp): Promise<void> {
  if (op.op === 'write') {
...
async function applyBlock(op: FileOp): Promise<void> {
  if (!op.blockId) throw new Error('append-block op missing blockId')

  const existing = await readFileSafe(op.destPath)
  const startMarker = `<!-- tam:start assetId=${op.blockId}`
  const endPattern = new RegExp(
    `<!-- tam:start assetId=${op.blockId}[^]*?<!-- tam:end assetId=${op.blockId}[^\n]*\n?`,
    'g'
  )
...
async function removeBlock(op: FileOp): Promise<void> {
  if (!op.blockId) return
  const existing = await readFileSafe(op.destPath)
  if (!existing) return
